...
const validatePropValue = (value, validationRule, propType, prop) => {
    ...
    if (typeof validationRule === 'object') {
        ...
        validationRule = validationRule.rule;
...
const validate = (entityData, schema, entityKind) => {
    ...
    for (let i = 0; i < totalProps; i += 1) {
        ...
        if (!skip) {
            ...
            // ... valid prop Value
            if (error === null && schemaHasProperty && !isEmpty && {}.hasOwnProperty.call(schema.paths[prop], 'validate')) {
    ...
    return {
        ...
        then: (resolve, reject) => {
            if (validationError) {
                return Promise.resolve(reject(validationError));
            }

            return Promise.resolve(resolve(entityData));
        },
        catch: (reject) => {
            if (validationError) {
                return Promise.resolve(reject(validationError));
