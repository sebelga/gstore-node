...
class Entity {
    ...
    plain(options) {
        ...
        const readAll = {}.hasOwnProperty.call(options, 'readAll') ? options.readAll : false;
        const virtuals = {}.hasOwnProperty.call(options, 'virtuals') ? options.virtuals : false;

        if (virtuals) {
            this.addVirtuals(this.entityData);
        }

        const data = datastoreSerializer.fromDatastore.call(this, this.entityData, { readAll });
...
function buildEntityData(self, data) {
    ...
    Object.keys(schema.paths).forEach((k) => {
        ...
        if (!{}.hasOwnProperty.call(entityData, k) &&
            ...
            let value = {}.hasOwnProperty.call(schemaProperty, 'default') ? schemaProperty.default : null;
