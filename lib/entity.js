...
function buildEntityData(self, data) {
    ...
    Object.keys(schema.paths).forEach((k) => {
        ...
        /**
         * There is an inconsistency in @google-cloud for Array type
         * We cannot (so far) use the Array way of declaring excludeFromIndexes
         * (like we can do with string, int and object)
         * ---> see serializer/datastore.js
         */
        ...
        if (prop.excludeFromIndexes === true && !isTypeArray) {
            ...
            self.excludeFromIndexes = [...self.excludeFromIndexes, ...excludedProps];
        }
    });
