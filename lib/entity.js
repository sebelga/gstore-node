...
const { addPathToPopulateRefs, populateFactory } = populateHelpers;
...
class Entity {
    ...
    // TODO: Move all entity method HERE
    populate(path, propsToSelect) {
        const paths = path ? arrify(path) : this.constructor.getEntitiesRefsFromSchema();
        const refsToPopulate = [];
        paths.forEach(p => addPathToPopulateRefs(p, propsToSelect, refsToPopulate));

        const promise = Promise.resolve()
            .then(() => this.constructor.populate(refsToPopulate)(this));
