...
const { key } = require('./helpers').googleCloud;
...
function createDataLoader(ds) {
    ...
    return new DataLoader((keys) => {
        const uuidKeys = keys.map(key.toString);
        const { length } = keys;

        const sortEntities = entities => entities.sort((a, b) => (
            uuidKeys.indexOf(key.toString(a[ds.KEY])) - uuidKeys.indexOf(key.toString(b[ds.KEY]))
        ));
        ...
        return ds.get(keys).then((res) => {
            ...
            if (length > entities.length) {
                ...
                const strIds = entities.map(entity => entity[ds.KEY]).map(key.toString);
    ...
    }, { cacheKeyFn: _key => key.toString(_key) });
}
