...
const { queryHelpers, validation, googleCloud } = require('./helpers');
...
class Model extends Entity {
    ...
    static get(id, ancestors, namespace, transaction, options, cb) {
        ...
        return cache.getKeys(key, this.gstore.cache)
            .then((result) => {
        ...
        /**
         * If a cache manager has been passed
         * to the gstore instance, we first check if the key
         * if present in its cache
         */
        // if (!multiple && this.gstore.cache) {
        //     return getFromCache().then(onEntity, onError);
        // }

        // return fetchEntity().then(onEntity, onError);
        ...
        function fetchEntity() {
            ...
            if (options.dataloader) {
                ...
                if (multiple) {
                    return options.dataloader.loadMany(key);
                }
                return options.dataloader.load(key);
            }

            return _this.gstore.ds.get(key);
        }
        ...
        function onEntity(data) {
            ...
            // Prime the cache
            // if (!multiple && _this.gstore.cache && !entity[0].entityData.__fromCache) {
            //     const cacheEntityData = Object.assign({}, entity[0].entityData, { __fromCache: true });

            //     return _this.gstore.cache.set(stringKey, cacheEntityData)
            //         .then(() => cb(null, entity[0]), (err) => {
            //             onError(err);
            //         });
            // }
    ...
    static query(namespace, transaction) {
        ...
        query.run = function runQuery(options, cb) {
            ...
            function onQuery(data) {
                ...
                if (info.moreResults !== _this.gstore.ds.NO_MORE_RESULTS) {
