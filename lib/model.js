...
class Model extends Entity {
    ...
    static get(id, ancestors, namespace, transaction, options, cb) {
        ...
        /**
         * First get from cache. If no cache has been set
         * it will forward the call to the fetchHandler along
         * with the original keys.
         * If there is a cache and it finds *some* keys but not all,
         * gstoreCache will call the fetch handler passing only the
         * missing keys and then set those keys in the cache before
         * merging all entities (catch + fetch).
         */
        return gstoreCache.getKeys(key, options, keys => fetchEntity(keys))
            .then(onEntity)
            .catch(onError);
        ...
        function onEntity(data) {
            ...
            let entity = arrify(data[0]);
            const isMultiple = entity.length > 1;
