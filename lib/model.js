    ...
    class Model extends Entity{
        ...
        static get(id, ancestors, cb) {
            ...
            this.ds.get(key, (err, entity) => {
                ...
                if (!entity) {
                    return cb({
                        code   : 404,
                        message: 'Entity {' + id.toString() + '} not found'
                    });
                }
        ...
        static update(id, data, ancestors, cb) {
            ...
            this.ds.get(key, (err, entity) => {
                ...
                let _Model = this.compile(_this.entityName, _this.schema, _this.ds);
                let model = new _Model(entity.data, id, ancestors);

                model.save(undefined, {emit:false}, (err, entity) => {
                    if (err) {
                        return cb(err);
                    }
        ...
        static delete(id, ancestors, cb) {
            ...
            this.ds.delete(key, (err, apiRes) => {
                if (err) {
                    return cb(err);
                }
                let success = apiRes.indexUpdates > 0;
                cb(null, success, apiRes);
            });
        }
        ...
        save (transaction, options, cb) {
            var args = arrayArguments(arguments);

            cb          = args.pop();
            transaction = args.length > 0 && args[0] !== null ? args[0] : undefined;
            options     = args.length > 1 ? args[1] : {emit:true};
            ...
            if (typeof transaction === 'undefined') {
                this.ds.save(entity, (err) => {
                    ...
                    entity.simplify = () => {
                        return datastoreSerializer.fromDatastore({key:_this.entityKey, data:_this.entityData});
                    };
