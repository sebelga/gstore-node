...
class Model extends Entity {
    ...
    static update(id, data, ancestors, namespace, transaction, options, cb) {
        ...
        if (typeof transaction === 'undefined' || transaction === null) {
            ...
            return transaction.run().then((transactionData) => {
        ...
        function saveEntity(getData) {
            ...
            return model.save(transaction, { op: 'update' });
        }

        function onEntityUpdated(updateData) {
            entityUpdated = updateData[0];
            infoTransaction = updateData[1];
        ...
        function onTransactionSuccess(transactionData) {
    ...
    static query(namespace, transaction) {
        ...
        query.run = function runQuery(options, cb) {
            ...
            function onQuery(data) {
                ...
                entities = entities.map(entity => datastoreSerializer.fromDatastore.call(_this,
                                                                                            entity,
                                                                                            options.readAll));
    ...
    static modelQuery(namespace, transaction) {
        const _this = this;

        const modelQuery = this.query(namespace, transaction);
        modelQuery._runInternal = modelQuery.run;

        modelQuery.run = function runQuery(options, cb) {
            const args = Array.prototype.slice.apply(arguments);
            cb = args.pop();

            options = args.length > 0 ? args[0] : {};

            modelQuery._runInternal(options, (err, response) => {
                if (err) {
                    cb(err);
                } else {
                    response.entities = response.entities.map((entity) => {
                        const key = entity[_this.gstore.ds.KEY];
                        delete entity.id;
                        return _this.__model(entity, null, null, null, key);
                    });
        ...
        modelQuery.run = utils.promisify(modelQuery.run);
    ...
    static list(options, cb) {
        ...
        function onSuccess(queryData) {
            ...
            // Add id property to entities and suppress properties
            // where "read" setting is set to false
            entities = entities.map(entity => datastoreSerializer.fromDatastore.call(_this, entity, options.readAll));
    ...
    static delete(id, ancestors, namespace, transaction, key, cb) {
        ...
        function onDelete(results) {
            ...
            if (typeof response.indexUpdates !== 'undefined') {
                success = response.indexUpdates > 0;
            }
            return cb(null, success, response);
        }
    ...
    static deleteAll(ancestors, namespace, cb) {
        ...
        function onEntities(data) {
            ...
            if ((_this.__pres && {}.hasOwnProperty.call(_this.__pres, 'delete'))) {
                ...
                return entities.reduce(chainPromise, Promise.resolve())
                                .then(onEntitiesDeleted, onError);
            }

            const keys = entities.map(entity => entity[_this.gstore.ds.KEY]);
    ...
    static excludeFromIndexes(properties) {
        ...
        properties.forEach((p) => {
            if (!{}.hasOwnProperty.call(this.schema.paths, p)) {
    ...
    static sanitize(data) {
        ...
        Object.keys(data).forEach((k) => {
            if (!{}.hasOwnProperty.call(this.schema.paths, k) || this.schema.paths[k].write === false) {
                delete data[k];
            } else if (data[k] === 'null') {
                data[k] = null;
            }
    ...
    save(transaction, options, cb) {
        ...
        if (cb) {
            return cb(null, _this, info, transaction);
        }
    ...
    validate(cb) {
        ...
        Object.keys(this.entityData).forEach((k) => {
            ...
            if (!skip && schemaHasProperty && !isValueEmpty &&
                ...
                errors[k] = new Error(`Value not allowed for ${k}.
                                        It must be one of: ${schema.paths[k].values}`);
            }
        });
