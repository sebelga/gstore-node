    ...
    class Model extends Entity{
        ...
        validate(cb) {
            ...
            Object.keys(this.entityData).forEach((k) => {
                // Properties dict
                if (!schema.paths.hasOwnProperty(k) && schema.options.unkwnownProperties === true) {
                    // No more validation, key does not exist but it is allowed
                    payload = {success:true};
                }

                if (!payload.success && !schema.paths.hasOwnProperty(k)) {
                    payload = {
                        success : false,
                        code:400,
                ...
                // Properties type
                if (!payload.success && self.entityData[k] !== null && schema.paths[k].hasOwnProperty('type')) {
                ...
                // Value Validation
                if (!payload.success && schema.paths[k].hasOwnProperty('validate') && self.entityData[k] && self.entityData[k] !== '') {
                ...
                // Preset values
                if (!payload.success && schema.paths[k].hasOwnProperty('values') && self.entityData[k] !== '') {
