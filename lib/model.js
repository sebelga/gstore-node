            ...
            Object.keys(this.entityData).forEach((k) => {
                ...
                if (!error && self.entityData[k] !== null && schema.paths[k].hasOwnProperty('type')) {
                    if (schema.paths[k].type === 'datetime') {
                        if (typeof self.entityData[k] !== 'string' || !moment(self.entityData[k]).isValid()) {
                            error = {
                                success: false,
                                code   : 400,
                ...
                // Validator
                if (!error && schema.paths[k].hasOwnProperty('validator') && self.entityData[k] !== '') {
                    if (!validator[schema.paths[k].validator](self.entityData[k])) {
                        error = {
                            success: false,
                            code   : 400,
