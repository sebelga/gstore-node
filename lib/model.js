...
class Model extends Entity {
    ...
    static get(id, ancestors, namespace, transaction, options, cb) {
         ...
         * the key(s) from it.
         * If the cache only returns *some* of the keys, gstore-cache
         * automatically calls the "fetchHandler" with only the keys that haven't
         * been found.
         * The final response is the merge of the cache + the fetch.
         */
        if (this.gstore.cache) {
            const fetchHandler = keys => fetchEntity(keys);
            return this.gstore.cache.keys.wrap(key, options, fetchHandler)
