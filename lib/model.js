...
const { addPathToPopulateRefs } = populateHelpers;
...
class Model extends Entity {
    ...
    static get(id, ancestors, namespace, transaction, options = {}) {
        ...
        const populate = (path, select) => {
            if (select && Array.isArray(path)) {
                throw new Error('Only 1 property can be populated when fields to select are provided');
            }

            // If no path is specified, we fetch all the schema props that are entities refs
            const paths = path
                ? arrify(path)
                : this.getEntitiesRefsFromSchema();

            paths.forEach(p => addPathToPopulateRefs(p, select, refsToPopulate));
    ...
    static populate(refs, options = {}) {
        ...
        const getPopulateMetaForEntity = (entity, entityRefs) => {
            ...
            entityRefs.forEach((ref) => {
                const { path } = ref;
                const key = get(entity.entityData, path);
                if (!key) {
                    set(entity.entityData, path, null);
                    return;
                }
                // Stringify the key
