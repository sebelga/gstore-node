...
class Query {
    ...
    initQuery(namespace, transaction) {
        ...
        query.run = function runQuery(options = {}) {
            ...
            return this.__originalRun.call(query, options).then(onQuery);
    ...
    findOne(params, ancestors, namespace, options) {
        ...
        return query.run(options).then(onSuccess);
    ...
    findAround(property, value, options, namespace) {
        ...
        return query.run(rest).then(({ entities }) => entities);
